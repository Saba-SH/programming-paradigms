...
Worker(void *threadarg)
{
  ...
  while (1) {
    ...
    case ACTION_DONE:
      ...
      break;
    case ACTION_DEPOSIT:
    // pthread_mutex_lock(&(Account_LookupByNumber(bank, action.u.depwithArg.accountNum)->useLock));
    ...
    //   pthread_mutex_unlock(&(Account_LookupByNumber(bank, action.u.depwithArg.accountNum)->useLock));
    ...
    // if(action.u.transArg.srcAccountNum<action.u.transArg.dstAccountNum){
    // pthread_mutex_lock(&(Account_LookupByNumber(bank, action.u.transArg.srcAccountNum)->useLock));
    // pthread_mutex_lock(&(Account_LookupByNumber(bank, action.u.transArg.dstAccountNum)->useLock));}
    // else{
    //     pthread_mutex_lock(&(Account_LookupByNumber(bank, action.u.transArg.dstAccountNum)->useLock));
    // pthread_mutex_lock(&(Account_LookupByNumber(bank, action.u.transArg.srcAccountNum)->useLock));
    // }
    ...
    //   pthread_mutex_unlock(&(Account_LookupByNumber(bank, action.u.transArg.srcAccountNum)->useLock));
    // pthread_mutex_unlock(&(Account_LookupByNumber(bank, action.u.transArg.dstAccountNum)->useLock));
