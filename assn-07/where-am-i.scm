...
;; get all the intersection points for the first circle in the list
(define (intersection-points-helper circles)
    (if (< (length circles) 2) '()
        (append (intersect (car circles) (cadr circles)) 
        (intersection-points-helper (cons (car circles) (cdr (cdr circles)))))))

;; get a list of all the intersection points of all the circles in the list
(define (intersection-points circles)
    (if (< (length circles) 2) '()
        (append (intersection-points-helper circles)
                (intersection-points (cdr circles)))))

;; get the distance product of a point
(define (distance-product pt ptlst)
    (apply * (map (lambda(pt1) (if (= 0 (dist pt pt1)) 1 (dist pt pt1)))  ;; lambda returns 1 if pt1 is same as pt, distance if it not
        ptlst)))

;; get a list of distance products of points in the list
(define (rate-points ptlst)
    (map (lambda(pt) (cons (distance-product pt (remove pt ptlst)) (list pt))) ptlst))

;;
;; updated version of partition. not limited to numbers
;; 
(define (partition-updated pivot lst cmp)
  (if (null? lst) '(() ())
      (let ((split-of-rest (partition-updated pivot (cdr lst) cmp)))
	(if (cmp (car lst) pivot)
	    (list (cons (car lst) (car split-of-rest)) (cadr split-of-rest))
	    (list (car split-of-rest) (cons (car lst) (car (cdr split-of-rest))))))))

;;
;; updated version of quicksort. is not limited to numbers as it accepts a compare function
;;
(define (quicksort-updated lst cmp)
    (if (< (length lst) 2) lst
        (let (
                (parts (partition-updated (car lst) (cdr lst) cmp))
             )
             (append
                (quicksort-updated (car parts) cmp)
                (list (car lst))
                (quicksort-updated (cadr parts) cmp)
             )
        ))
)

;; sort points in ascending order of rating
(define (sort-points ratlst)
    (quicksort-updated ratlst
        (lambda (rat0 rat1) 
            (< (car rat0) (car rat1))
        )
    )
...
;; get lower half of distance ratings
(define (clumped-points ptlst)
    (map cadr 
        (prefix-of-list 
            (sort-points (rate-points ptlst)) 
            (quotient (length ptlst) 2)
        )
    )
)

;; gets a point with average coordinates based on list, as well as its distance rating
(define (average-point ptlst)
    (let (
            (aver (make-pt (/ (apply + (map car ptlst)) (length ptlst)) (/ (apply + (map cadr ptlst)) (length ptlst))))
         )
         (cons (distance-product aver ptlst) (list aver))
    )
)

;; gets the best estimate of our current location based on given star positions
(define (best-estimate circles)
    (average-point
        (clumped-points
            (map cadr
                (sort-points 
                    (rate-points (intersection-points circles))
                )
            )
        )
    )
...
;; generates all guesses of star positions and determines our current location based on best estimates of guesses
